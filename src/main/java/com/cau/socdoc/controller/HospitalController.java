...
public class HospitalController {
    ...
    public ApiResponse<ResponseDetailHospitalDto> findHospitalDetailById(@PathVariable String hospitalId, @RequestParam String userId) throws ExecutionException, InterruptedException {
    ...
    // 특정 지역의 특정 분과 병원 조회
    @Operation(summary = "특정 지역의 특정 분과 병원 조회", description = "특정 지역의 특정 분과 병원 목록을 조회합니다. (예: 서울시 관악구의 치과)")
    ...
    // 특정 지역의 병원 조회
    @Operation(summary = "특정 지역의 병원 조회", description = "특정 지역의 병원 목록을 조회합니다. (예: 서울시 동작구의 병원)")
    @GetMapping("/list")
